% Generate LFP values for simulation of the full network (With 15:1 GC:MC
% ratio)
...
for trial = 1:numTrials
    ...
    fname_1 = sprintf('LFPresultsSM6_results_trial3%d.mat',trial);
...
function [mOdor_Amp, mOdor_Phase, gloms] = odorGenerator(glomeruli, glomArray, mitralNum, glomNum)
    ...
    for i = 1:length(glomInt)
        ...
        if i <= glomNum
            ...
            % input strength
            meanInput = rand*0.25; % original baseline is 0.5
            mOdor_Amp(R) = normrnd(meanInput,meanInput/10,length(R),1);
