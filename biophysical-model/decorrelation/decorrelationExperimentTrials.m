% Calculates the decorrelation induced by the granule cell network for different pairs of odors
% by first simulating each odor with and then without the GC network active
...
numOdors = 6;
spikeTrainRecord = cell(2,numOdors);
...
FB_cells = GC_order(1:FBnum);
gFeedback1(FB_cells) = rand(length(FB_cells),1) * 200;
...
odorMatrixAmp = zeros(mitralNum, numOdors); 
odorMatrixPhase = zeros(mitralNum, numOdors); 
odorIndices = [1:30; 6:35; 11:40; 16:45;21:50;26:55];
for t = 1:numOdors
    for i = 1:length(glomeruli)
        R = find(glomArray == i);
        if sum(ismember(i, odorIndices(t,:)))
            odorMatrixAmp(R,t) = odor_Amp_basis(R);
            odorMatrixPhase(R,t) = odor_Phase_basis(R);
        else
            mean_phase = 2*pi*rand;
            meanInput = rand*150;
            odorMatrixAmp(R,t) = normrnd(meanInput,meanInput/5,length(R),1);
            odorMatrixPhase(R,t) = normrnd(mean_phase,pi/4,length(R),1);
        end
    end
end
...
for r = 1:2
    if r == 2
        mGABA = normrnd(0,0/10,mitralNum,granuleNum).* exp(-distance./Lmat); % GABA adjusted for distance
    end
    
    for trial = 1:numOdors
        beginOdor = 0; % begin odor after 1 s of breathing
        beginFB = 0;
        endOdor = beginOdor + 6/fodor; % end odor after 1 s of odor

        [spikeTrainRecord{r, trial}, ~, ~, ~]  = simulator(network, mParam, gParam,...
            mResp_Amp, mResp_Phase, fnorm, odorMatrixAmp(:,trial), odorMatrixPhase(:,trial), fodor, zeros(granuleNum,1), beginOdor, beginFB, endOdor,...
    ...
    %         tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx);
        ...
        fname = sprintf('DCresultsTrial.mat');
...
function [mOdor_Amp, mOdor_Phase, gloms] = odorGenerator(glomInt, glomArray, mitralNum, glomNum)
