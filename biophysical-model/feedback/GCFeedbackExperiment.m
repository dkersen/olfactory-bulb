% Simulates the effects of external excitatory feedback to GCs for
% different fractions of GCs

rng shuffle

% Load network and distance matrices and glomeruli
load('fullNetwork.mat')
load('fullNetworkAltalt.mat')
load('distance.mat')
load('distanceAltalt.mat')
load('glomeruli.mat')
...
mGABAlt = normrnd(0.13,0.13/10,mitralNum,granuleNum).* exp(-distanceAlt./Lmat); % GABA adjusted for distance
...
% GC cortical feedback parameters
...
feedback_frac = [0.001, 0.01, 0.05, 0.1, 0.15, 0.2];

numTrials = length(feedback_frac);
spikeTrainRecord = cell(4,numTrials);
feedbackRecord = cell(1,numTrials);

% set the respiratory rates
fnorm = 2/1000; % 2 Hz breathing rate
fodor = 6/1000; % 6 Hz sniff rate during odor presentation
    fileID = fopen('FEFresults.txt','w');

for trial = 1:numTrials
    FB_frac = feedback_frac(trial);
    FBnum = round(FB_frac*granuleNum);
    
    gFeedback1 = zeros(granuleNum,1);   
    gFeedback2 = zeros(granuleNum, 1);
    GC_order = randperm(granuleNum);
    FB_cells_1 = GC_order(1:FBnum);
    FB_cells_2 = GC_order(FBnum+1:2*FBnum);
    gFeedback1(FB_cells_1) = 50;
    gFeedback2(FB_cells_2) = 50;
    feedbackRecord{trial} = [gFeedback1, gFeedback2];

    beginOdor = 0; % begin odor after 1 s of breathing
    beginFB = 0;
    endOdor = beginOdor + 2/fodor; % end odor after 1 s of odor

    [spikeTrainRecord{1, trial}, ~, ~, ~]  = simulator(network, mParam, gParam,...
        mResp_Amp, mResp_Phase, fnorm, mOdor_Amp, mOdor_Phase, fodor, zeros(granuleNum,1), beginOdor, beginFB, endOdor,...
    ...
    [spikeTrainRecord{3, trial}, ~, ~, ~]  = simulator(network, mParam, gParam,...
        mResp_Amp, mResp_Phase, fnorm, mOdor_Amp, mOdor_Phase, fodor, gFeedback2, beginOdor, beginFB, endOdor,...
        mGABA, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
        tauA_g, tauNr_g, tauNd_g, gAMPA, gNMDA,...
        tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx); 
    
    [spikeTrainRecord{4, trial}, ~, ~, ~]  = simulator(networkAlt, mParam, gParam,...
        mResp_Amp, mResp_Phase, fnorm, mOdor_Amp, mOdor_Phase, fodor, zeros(granuleNum,1), beginOdor, beginFB, endOdor,...
        mGABAlt, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
        tauA_g, tauNr_g, tauNd_g, gAMPA, gNMDA,...
        tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx); 
    
    [spikeTrainRecord{5, trial}, ~, ~, ~]  = simulator(networkAlt, mParam, gParam,...
        mResp_Amp, mResp_Phase, fnorm, mOdor_Amp, mOdor_Phase, fodor, gFeedback1, beginOdor, beginFB, endOdor,...
        mGABAlt, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
        tauA_g, tauNr_g, tauNd_g, gAMPA, gNMDA,...
        tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx); 
    ...
    fprintf(fileID, formatSpec, trial);
    
    fname = sprintf('GCFeedbackResults.mat');
    save(fname,'spikeTrainRecord','feedbackRecord', 'odorGloms','-v7.3');
end
