% Simulates the biophysical model to acquire firing rate distributions in
% response to odor and in response to direct MC feedback
...
for i = 1:length(glomeruli)
    ...
    % phase of input
    mResp_Phase(R) = normrnd(pi/2,pi/4,length(R),1);
end
...
for i = 1:numOdors
    [mOdor_Amp, mOdor_Phase, odorGloms{i}] = odorGenerator(glomeruli, glomArray, mitralNum, odorGlomNum);
   
    mFeedback = zeros(mitralNum,1);
    FB_frac = 0.2;
    FBnum = round(FB_frac*mitralNum);
    
    MC_order = randperm(mitralNum);
    FB_cells = MC_order(1:FBnum);
    mFeedback(FB_cells) = rand(length(FB_cells),1)*300; %100 and 10 originally

    [spikeTrainRecord{1,i}, ~, ~, ~]  = simulator(network, mParam, gParam,...
        mResp_Amp, mResp_Phase, fnorm, mOdor_Amp, mOdor_Phase, fodor, zeros(mitralNum,1), beginOdor, beginFB, endOdor,...
        mGABA, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
        tauA_g, tauNr_g, tauNd_g, gAMPA, gNMDA,...
        tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx);
    [spikeTrainRecord{2,i}, ~, ~, ~]  = simulator(network, mParam, gParam,...
        mResp_Amp, mResp_Phase, fnorm, mOdor_Amp, mOdor_Phase, fodor, mFeedback, beginOdor, beginFB, endOdor,...
        mGABA, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
        tauA_g, tauNr_g, tauNd_g, gAMPA, gNMDA,...
        tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx);
    feedbackRecord{i} = mFeedback;
    fname = sprintf('MCdistributionR.mat');
...
function [mOdor_Amp, mOdor_Phase, gloms] = odorGenerator(glomeruli, glomArray, mitralNum, glomNum)
    ...
    for i = 1:length(glomInt)
        ...
        if i <= glomNum
            ...
            % input strength
            meanInput = 300 + rand*100; %400 + rand*200
            mOdor_Amp(R) = normrnd(meanInput,meanInput/5,length(R),1);

            % phase of input
            mOdor_Phase(R) = 2*pi*rand(length(R),1);
        else
            % input strength
            meanInput = rand*0;
            mOdor_Amp(R) = normrnd(meanInput,meanInput/5,length(R),1);
