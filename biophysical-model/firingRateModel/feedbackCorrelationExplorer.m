% Examines how the slope of the relationship between initial similarity and
% change in similarity varies as a function of the similarity of the feedback patterns 
...
targetAct = 0.01;
cellOverlap = 0:0.01:1;

slopeResult = zeros(1,length(cellOverlap));
fbsim = zeros(1,length(cellOverlap));



for num = 1:length(cellOverlap)
        ...
        number_overlap_ratio = cellOverlap(num);
        ...
        %     odor_responses_2(overlap_order(1:round(same_odor_fraction*mitralNum))) = odor_responses(overlap_order(1:round(same_odor_fraction*mitralNum)));
            ...
            if strcmp(feedback_type, 'positive positive') == true %feedback targets same MCs with same positive strength
                ...
                overlap_num = round((1-number_overlap_ratio)*FB_num);
        ...
        slopeResult(num) = coefs(1);
        fbsim(num) = cosine_distance(feedback_1', feedback_2');
        save('FCEresults', 'slopeResult','fbsim');        
end
