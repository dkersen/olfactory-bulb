% Determines the strength of lateral inhibition by simulating firing of one
% and then two cells in the network to see how the firing of the second
% cell affects the first

rng shuffle

network = load('fullNetwork.mat');
network = network.('network');
sliceDistance = load('distance.mat');
...
xsamp = -0.668:0.0001:25;
c = 0.8125;
f = -0.1957;
g = 0.1145;
m = 0.7118;
r = 0.6509;
curve = -exp(-g*xsamp)*f*g + (m*r)./(1+(c+m*xsamp).^2);
...
R = randpdf(curve,xsamp,[mitralNum 1]);
phase = normrnd(pi/2,pi/4,mitralNum,1);
        
f = @(x)networkCost(x,network, sliceDistance, phase, R,Lmat, mParam, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
    gParam, tauA_g, tauNr_g, tauNd_g,... 
    mitPairs, networks,mParameters, gParameters, removals);
...
diff = networkCost(x0,network, sliceDistance, phase, R, mParam, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
gParam, tauA_g, tauNr_g, tauNd_g,... 
mitPairs, networks, mParameters, gParameters, removals);

save('differences','diff')
...
function diff = networkCost(x,network, paraDistance, phase, R, mParam, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
    gParam, tauA_g, tauNr_g, tauNd_g,... 
    mitPairs, networks, mParameters, gParameters, removals)
    ...
    diff = sliceCost(x, mitPairs, networks, mParameters, gParameters, gAMPA, gNMDA); %...
%         + baseCost(x, mGABA, gAMPA, gNMDA, network,phase, R, mParam, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
%     gParam, tauA_g, tauNr_g, tauNd_g); 
    %save('currentValue2','x');
end

function diffArray = sliceCost(x, mitPairs, networks, mParameters, gParameters, gAMPA, gNMDA)
    [len,~] = size(mitPairs);
    diffArray = zeros(1,len);
%     inputArray = [700 700  680 680 680
%                   130 130  130 130 120 
%                   1280 1300 1250 1250 1250];
    fileID = fopen('LIresults.txt','w');
    for i = 1:len
        ...
        diffArray(i) = freq1 - freq2;
        formatSpec = 'Number is %d\n';
...
function f1 = latInhib(x, I1, I2, network, numFlag, mParam, mGABA, tauG_m, gParam, tauA_g, tauNr_g, tauNd_g, gAMPA, gNMDA)
    ...
    numRecN = 1;
    numRecA = 1;
    mX = zeros(mitralNum, numRecN);
    mIntegN = zeros(mitralNum, numRecN);
    mIntegA = zeros(mitralNum, numRecA);
