%Generates a network of MC and GCs with reduced GC:MC ratio (5:1)
...
%     while d > rmax
%         glomerX = -rmax + 2*rand*rmax;
%         glomerY = -rmax + 2*rand*rmax;
%         d = norm([glomerX, glomerY]);
%     end
    r = rmax*sqrt(rand);
    theta = 2*pi*rand;
    glomerX = r*cos(theta);
...
save('mitralCellsSmol.mat', 'mitralArray','-v7.3');
save('fullNetworkSmol.mat', 'network', '-v7.3');
save('granuleCellsSmol.mat', 'granuleArray', '-v7.3');
save('distanceSmol.mat', 'distance','-v7.3');
save('glomeruliSmol.mat', 'glomXYarray', 'glomArray','-v7.3');
