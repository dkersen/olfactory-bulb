% Conducts the neurogenesis experiment for the guided addition mode
...
baseNetwork = network;
baseGranuleNum = granuleNum;
baseMitralNum = mitralNum;
...
for trial = 1:5
    ...
    mitralNum = baseMitralNum;
    granuleNum = baseGranuleNum;
    network = baseNetwork;
    ...
    % impatient like me
    fileID = fopen('guidedResults.txt','w');
    ...
    for gene = 1:numGeneses   
        ...
        %         tauAx_g, tauNrx_g, tauNdx_g, gAMPAx, gNMDAx);
            ...
            spikeTotal = spikeTotal + sum(spikeTrainRecord{gene,odor}(:,1668:end),2);
        end

        totalSpikes = sum(spikeTotal);

        spikeDist = zeros(1,totalSpikes);
        counter = 0;
        for i = 1:mitralNum
            while spikeTotal(i) > 0
                counter = counter+1;
                spikeDist(counter) = i;
                spikeTotal(i) = spikeTotal(i) - 1;
            end
        end
        ...
        if gene < numGeneses
            ...
            while addedNum <= targetNum
                ...
                newGranule = granule_direct();
                index = randi(totalSpikes);
                mc = mitralArray(spikeDist(index));
                newGranule = newGranule.assignProperties(rmax, mc);
        ...
        fname = sprintf('guidedResults%d.mat', trial);
